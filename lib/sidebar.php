...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?> 
		</p>	<li><?php echo ('L\'utilisateur qui a réalisé le plus de distance ce mois-çi est '.htmlspecialchars($donnees['user']). ' avec ' .htmlspecialchars($donnees['maxdistmonth']).' kilometres') ;?></li>			
<?php 		
<p>
...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?>
	</p>		<li><?php echo ('Le nageur du mois est '.htmlspecialchars($donnees['user']). ' qui a passé ' .htmlspecialchars($donnees['maxdureenatation']).' minutes dans l\'eau') ;?></li>
<?php 		
<p>

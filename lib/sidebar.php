...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?>
			<li><?php echo ('Notre champion cycliste du mois de '.date('F').' est '.htmlspecialchars($donnees['user']). ' qui a passe ' .htmlspecialchars($donnees['maxdureecyclisme']).' minutes sur sa selle, il doit avoir sacrement mal au cul!') ;?></li>
<?php 		
...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?>
			<li><?php echo (htmlspecialchars($donnees['user']). ' est celui qui a usé le plus ces chaussures en '.date('F').' avec ' .htmlspecialchars($donnees['maxdureecourse']).' minutes de course a pied') ;?></li>

...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?>
		...
		<ul>
			<li>Nous sommes le <?php echo date('l jS F Y');?><br></li>
...
<?php		if ($reponse != False) $donnees = mysql_fetch_array($reponse); ?> 
			<li><?php echo ('L\'utilisateur qui a réalisé le plus de distance ce mois-ci est '.htmlspecialchars($donnees['user']). ' avec ' .htmlspecialchars($donnees['maxdistmonth']).' kilometres') ;?></li>			
<?php 		
